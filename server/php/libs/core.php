...
function sendMail($template, $replace_content, $to, $reply_to_mail = '')
{
    ...
    if ($template) {
        ...
        $mail->Host = R_SMTP_HOSTNAME;  // Specify main and backup SMTP servers
        if (strlen(R_SMTP_USERNAME)){
            $mail->SMTPAuth = true;                               // Enable SMTP authentication
            $mail->Username = R_SMTP_USERNAME;                 // SMTP username
            $mail->Password = R_SMTP_PASSWORD;                           // SMTP password
        ...
        $mail->Port = R_SMTP_PORT;                                    // TCP port to connect to

        $mail->setFrom(DEFAULT_FROM_EMAIL_ADDRESS);
        if (!empty($reply_to_mail)) {
            $mail->AddReplyTo($reply_to_mail);
        }
        $mail->addAddress($to);
        ...
        $mail->addCustomHeader("X-Mailer: Restyaboard (0.2.1; +http://restya.com/board)");
